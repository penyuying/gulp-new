...
//                    }
            ...
            getImgPath: function () { //获取图片处理参数配置
                var pngobj = this.setObj("imgFile", "imgDstDir", "png", ".{png,PNG}");
                var jpgobj = this.setObj("imgFile", "imgDstDir", "jpg", ".{jpg,JPG}");
                var gifobj = this.setObj("imgFile", "imgDstDir", "gif", ".{gif,GIF}");
                var retGSrc =[];
                ...
                if (this.indexObj.copyPath.cfgArr.length > 0) {
                    ...
                    subMerge.add(this.indexObj.copyPath.cfgArr.map(function(cfg) {
                        ...
                        return PY.gulp.src(cfg.srcPath)
							...
							.pipe(PY.gulp.dest(cfg.destPath))
							.pipe(PY.gulpif(cfg.srcRev === true, PY.gulprev.manifest({path:"rev-manifest"+i+".json"})))
